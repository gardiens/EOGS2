	...
	const bool debug)
{
  ...
  // Pierrick
  torch::Tensor dL_dT= torch::zeros({P, 6}, means3D.options());
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations,dL_dT);
}
