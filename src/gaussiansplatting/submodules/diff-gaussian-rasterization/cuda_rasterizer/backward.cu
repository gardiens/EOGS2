	...
	float* dL_dT)
{
	...
	// Affine matrices do not project => Jacobian of non-linear transformation is the identity
	glm::mat3 T = W * NDC2Screen;
	...
	// In Affine Camera, the mean of a gaussian does not affect the 2D covariance matrix
	...
	float3 dL_dmean = {0.0, 0.0, 0.0};
	// Gradients of loss w.r.t. Gaussian means, but only the portion
	// that is caused because the mean affects the covariance matrix.
	// Additional mean gradient is accumulated in BACKWARD::preprocess.
	dL_dmeans[idx] = dL_dmean;

	// Added by pierrick we fetch the DL_DT
	dL_dT[idx+0]=dL_dT00;
	dL_dT[idx+1]=dL_dT01;
	dL_dT[idx+2]=dL_dT02;
	dL_dT[idx+3]=dL_dT10;
	...
	float* dL_dopacity)
{
	...
	glm::vec3 dL_dmean;
	dL_dmean.x = proj[0] * dL_dmean2D[idx].x + proj[1] * dL_dmean2D[idx].y;
	dL_dmean.y = proj[4] * dL_dmean2D[idx].x + proj[5] * dL_dmean2D[idx].y;
	dL_dmean.z = proj[8] * dL_dmean2D[idx].x + proj[9] * dL_dmean2D[idx].y;
